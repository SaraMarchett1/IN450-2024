...
A = CirculantL[IntegerDigits[FromDigits["8F",16],2,8]];
Print[A];
...
(* A e'auto-inversa ?  *)
Ainverse = Inverse[A, Modulus -> 2];
(* A^2 = I ? *)
Print["Test if A is A inverse : ", Mod[A.Ainverse,2] == IdentityMatrix[8] ];

(* Definizione della funzione g^-1 *)
(* Costruiamo A come una matrice circolante generata dal vettore v=(1,1,0,1) *)

Invgg[x_]:= FromDigits[Mod[Ainverse.(IntegerDigits[x, 2, Length@A]+b), 2], 2];
...
fj[c_, j_]:= Module[{px, x, rc, out},
	px = x^4+x+1;
	rc = FromDigits[Reverse@CoefficientList[PolynomialRemainder[x^(j/4 - 1), px, x, Modulus->2], x, 8], 2];
